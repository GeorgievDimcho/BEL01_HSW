...
double summe_berechnen(Artikel artikel[], int warenkorb[], int anzahl)
{
    ...
    for (int i = 0; i < anzahl; i++)
    {
        if (warenkorb[i] > 0)
        {
            summe = summe + (artikel[i].preis*warenkorb[i]);
        }
    }
    return summe;
}
...
int main(void)
{
    ...
    strcpy(artikel[0].name, "BILLY"); // number 1
    // artikel[0].name="BILLY";
    artikel[0].preis = 24.99;
    strcpy(artikel[1].name, "KALLAX"); // number 2
